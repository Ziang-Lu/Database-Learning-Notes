...
#### 行锁 (Row-level Lock) vs 表锁 (Table-level Lock)
...
| 比较项目 | 读锁 / 共享锁                                                | 写锁 / 排他锁                                               |
| -------- | ------------------------------------------------------------ | ----------------------------------------------------------- |
| 定义     | 当前事务获取某一行的读锁 (共享锁) 后, 可以对于该行进行R操作  | 当前事务获取某一行的写锁 (排他锁) 后, 可以对于该行进行W操作 |
|          | 与此同时, 其他事务对于同一行数据, 可以共享同一把读锁, 都能对该行进行R操作, 但会阻塞W操作 | W操作完成前, 阻塞其他所有对于该行的操作, 无论R还是W         |
...
-- 与此同时, 其他事务对于同一行数据, 可以共享同一把读锁, 都能对该行进行R操作, 但会阻塞W操作
```
...
### 2. Multi-Version Concurrency Control (MVCC) 多版本的并发访问控制 (`MySQL`默认方式)
...
* **`Repeatable Read`事务隔离级别:**
  ...
  * 当一个<u>事务开启时, 其看到一个特定版本的DB, 事务中的全部操作只能看到这个特定版本的DB, 所有的修改也只能对于这个特定版本的DB进行</u>

  * (=> 不同事务可以并发地进行, 而不用被彼此锁住)

    *(一想便知, 不会有"脏读"和"不可重复读"的问题)*

* **`Read Committed`事务隔离级别:**

  * <u>在一个事务中, 每一次`select`会看到一个特定版本的DB, 后续到下一次`select`之前, 事务中的全部操作只能看到这个特定版本的DB, 所有的修改也只能对于这个特定版本的DB进行</u>

    *(一想便知, 不会有"脏读"的问题, 但是自然会带来"不可重复读"的问题)*
...
| <u>实现方式</u>                          | 标准的:<br>R不阻塞R, 阻塞W<br>W阻塞R/W         | <u>MVCC</u>             |
| <u>对于`Repeatable Read`事务隔离级别</u> | <u>(标准的)</u><br>R不阻塞R, 阻塞W<br>W阻塞R/W | <u>所有事务互不阻塞</u> |
...
* 在`MySQL`中设置事务隔离级别为`Repeatable Read`
  * 若需要的并发量大, 选择使用"悲观锁", 靠具体的锁机制来实现并发访问控制 (LBCC的行锁机制实现)
